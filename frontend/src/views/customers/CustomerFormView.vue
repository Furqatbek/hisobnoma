...
onMounted(async () => {
  if (isEdit.value) {
    ...
    // Fetch next auto-generated code for preview
    try {
      const response = await customersApi.getNextCode()
      nextCode.value = response.data
    } catch (error) {
      ...
      <div class="card">
        ...
        <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input v-model="form.code" type="text" class="input" :placeholder="nextCode || $t('customers.form.autoGenerated')" :disabled="isEdit" />
            <p v-if="!isEdit && !form.code && nextCode" class="text-xs text-gray-400 mt-1">
              {{ $t('customers.form.autoGenerated') }}: {{ nextCode }}
            </p>
          </div>
          <div>
